...
export class PaypalOrderResponse {
    ...
    @ManyToMany(() => Link)
    @JoinTable({ name: "paypalOrderResponse_links" })
    links: Link[];
}
@Entity()
export class Link {
...
export class PaypalCaptureOrderResponse {
    ...
    @JoinTable({ name: "paypalCaptureOrderResponse_purchaseUnit" })
    purchase_units: PurchaseUnit[];
    @ManyToOne(() => Payer)
    payer: Payer;
    @ManyToMany(() => Link2)
    @JoinTable({ name: "paypalCaptureOrderResponse_Link2" })
    links: Link2[];
}
@Entity()
export class Name2 {
...
export class Payments {
    ...
    id: number;
    @ManyToMany(() => Capture)
    @JoinTable({ name: "payments_capture" })
    captures: Capture[];
}
@Entity()
