...
export class UploadXlService {
    ...
    async getVariations(row: any, headerRow: any): Promise<{ attribute_value_id: number }[]> {
        ...
        while (row[headerRow.indexOf(`Attribute ${i} Name`)] !== undefined && row[headerRow.indexOf(`Attribute ${i} Value`)] !== undefined) {
            ...
            if (attributeName && attributeValue) {
                for (const element of attributeValue) { // Use for...of loop
    ...
    async parseAttributes(row: any, headerRow: any): Promise<Record<string, number[]>> {
    ...
    async createVariationOptions(row: any, headerRow: any): Promise<VariationOptionDto[]> {
        ...
        while (row[headerRow.indexOf(`Attribute ${i} Name`)] !== undefined && row[headerRow.indexOf(`Attribute ${i} Value`)] !== undefined) {
            ...
            if (attributeName && attributeValue) {
                for (const element of attributeValue) { // Use for...of for async/await
        ...
        try {
            ...
            if (createProductDto.variations) {
                try {
                    ...
                    const attributeValueMap = new Map<number, AttributeValue>(attributeValues.map(attr => [attr.id, attr]));
                    ...
                    if (error instanceof NotFoundException) {
                        ...
                        throw new InternalServerErrorException('An error occurred while processing variations');
                    }
                }
            } else {
