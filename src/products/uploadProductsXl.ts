...
export class UploadXlService {
    ...
    async getOrCreateAttributeValue(attributeValue: string): Promise<AttributeValue> {
        const attrValue = await this.findAttributeValue(attributeValue);
        if (!attrValue) {
            throw new NotFoundException(
                `Attribute value '${attributeValue}' not found.`,
            );
        }
        return attrValue;
    }
    ...
    async getOrCreateAttributeValues(attributeValues: string[]): Promise<AttributeValue[]> {
        const promises = attributeValues.map((attrValue) =>
            this.getOrCreateAttributeValue(attrValue),
        );
    ...
    async createVariation(row: any, headerRow: any): Promise<any> {
        ...
        return {
            ...
            quantity: parseInt(row[headerRow.indexOf('Child Inventory')], 10),
    ...
    async getVariations(row: any, headerRow: any): Promise<{ attribute_value_id: number }[]> {
        ...
        while (
            ...
            const attributeValueString = row[headerRow.indexOf(`Attribute ${i} Value`)];

            if (attributeName && attributeValueString) {
                // Split the attribute values
                const attributeValues = await this.splitAttributeValues(attributeValueString);

                // Retrieve each attribute value
                for (const attributeValue of attributeValues) {
                    const fetchedAttributeValue = await this.findAttributeValue(attributeValue);

                    if (!fetchedAttributeValue) {
                        // If not found, handle the case accordingly, e.g., throw an error or log a warning
    ...
    async createVariationOptions(
        ...
        while (
            ...
            const attributeValue = row[headerRow.indexOf(`Attribute ${i} Value`)];
            if (attributeName && attributeValue) {
                const variationOption: VariationOptionDto = {
                    id: null, // or undefined if you prefer
                    name: attributeName,
    ...
    async saveProducts(createProductDto: CreateProductDto): Promise<Product> {
        try {
            ...
            if (createProductDto.variations) {
                const uniqueVariations = new Set(); // To store unique attribute combinations
                ...
                for (const variation of createProductDto.variations) {
                    ...
                    const uniqueKey = `${product.id}-${attributeValue.id}`;
                    if (!uniqueVariations.has(uniqueKey)) {
                        uniqueVariations.add(uniqueKey);
